...
import { IInvoice, IInvoiceItem, IInvoiceItemTax, ITaxTotal, IPayment } from '@sri/core';

function addTaxTotals(parent: any, taxes: ITaxTotal[]): void {
...
export function buildInvoiceXml(data: IInvoice): string {
  ...
  data.items.forEach((item: IInvoiceItem) => {
    ...
    if (item.taxes && item.taxes.length > 0) {
      ...
      item.taxes.forEach((tax: IInvoiceItemTax) => {
  ...
  if (data.payments && data.payments.length > 0) {
    ...
    data.payments.forEach((payment: IPayment) => {
  ...
  if (data.additionalInfo && Object.keys(data.additionalInfo).length > 0) {
    ...
    Object.entries(data.additionalInfo).forEach(([key, value]) => {
      additional.ele('campoAdicional', { nombre: key }).txt(String(value));
    });
  }
